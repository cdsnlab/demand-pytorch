...
class TemporalHeteroModel(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        loss = self.b_xent(logits, self.lbl[:n])
        return loss
...
class STSSLModel(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = STEncoder(Kt=3, Ks=3, blocks=[[2, int(config.d_model//2), config.d_model], [config.d_model, int(config.d_model//2), config.d_model]], 
                        input_length=config.input_length, num_nodes=config.num_nodes, droprate=config.dropout)
        
        # traffic flow prediction branch
        self.mlp = MLP(config.d_model, config.d_output)
        # temporal heterogenrity modeling branch
        self.thm = TemporalHeteroModel(config.d_model, config.batch_size, config.num_nodes, config.device)
        # spatial heterogenrity modeling branch
        self.shm = SpatialHeteroModel(config.d_model, config.nmb_prototype, config.batch_size, config.shm_temp)
    ...
    def forward(self, view1, graph):
        ...
        graph2 = aug_topology(s_sim_mx, graph, percent=self.config.percent*2)
        
        t_sim_mx = self.fetch_temporal_sim()
        view2 = aug_traffic(t_sim_mx, view1, percent=self.config.percent)
    ...
    def pred_loss(self, z1, z2, y_true, scaler):
        ...
        loss = self.config.yita * self.mae(y_pred[..., 0], y_true[..., 0]) + \
                (1 - self.config.yita) * self.mae(y_pred[..., 1], y_true[..., 1])
        return loss
