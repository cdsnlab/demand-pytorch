...
class STMGCNModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        # n_nodes:int, input_dim:int, lstm_hidden_dim:int, lstm_num_layers:int,
        ...
        for m in range(self.M):
            cglstm = CG_LSTM(seq_len=config.num_his, n_nodes=config.num_nodes, input_dim=config.input_dim,
                             lstm_hidden_dim=config.lstm_hidden_dim, lstm_num_layers=config.lstm_num_layers,
                             K=self.sta_K, gconv_use_bias=config.gconv_use_bias, gconv_activation=nn.ReLU)
            self.rnn_list.append(cglstm)
            gcn = GCN(K=self.sta_K, input_dim=config.lstm_hidden_dim, hidden_dim=config.gcn_hidden_dim,
                      bias=config.gconv_use_bias, activation=nn.ReLU)
            self.gcn_list.append(gcn)
        self.fc = nn.Linear(in_features=config.gcn_hidden_dim, out_features=config.input_dim, bias=True)
    ...
    def forward(self, obs_seq:torch.Tensor, sta_adj_list:list):
        ...
        output = self.fc(feat_fusion).unsqueeze(1)
        return output
